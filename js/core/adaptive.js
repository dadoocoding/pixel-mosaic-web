...
/**
 * Group quadtree leaves (see buildQuadtree) by their exact averaged color
 * and report how many grid cells each one covers in total -- the adaptive-
 * mode equivalent of colorCounts.js. Unlike classic mode there's no fixed
 * palette to number against, so results are sorted by cell count
 * descending (most-used color first) instead. Returns a list of
 * {hex, rgb, count}.
 */
export function adaptiveColorCounts(leaves) {
  const totals = new Map();
  for (const leaf of leaves) {
    const key = leaf.rgb.join(",");
    const entry = totals.get(key);
    const area = leaf.w * leaf.h;
    if (entry) entry.count += area;
    else totals.set(key, { rgb: leaf.rgb, count: area });
  }
  return [...totals.values()]
    .sort((a, b) => b.count - a.count || a.rgb.join(",").localeCompare(b.rgb.join(",")))
    .map(({ rgb, count }) => ({ hex: rgbToHex(rgb), rgb, count }));
}
